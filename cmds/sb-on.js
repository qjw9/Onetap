  ...
  async execute(message, args, client, db) {
    ...
    db.get('SELECT owner_id FROM temp_channels WHERE channel_id = ? AND guild_id = ?',
      [vc.id, guild.id], async (dbErr, row) => {
        ...
        try {
          await vc.permissionOverwrites.edit(guild.roles.everyone, { [PermissionsBitField.Flags.UseSoundboard]: true });
          return styledSend(message, { title: 'Success', emoji: e.sbOn.trim(), msg: 'Soundboard has been enabled for everyone' });
