...
module.exports = {
  ...
  async execute(message, args, client, db) {
    ...
    if (!config?.hide_role)
      return err('No hide role configured. Ask an admin to use `.v set-hide @role`.');
    ...
    try {
      ...
      return styledSend(message, { title: 'Success', emoji: e.success.trim(), msg: 'Your voice room is now hidden from regular members' });
    } catch {
      return err('Failed to hide the channel.');
    }
  },
};
