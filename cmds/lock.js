...
module.exports = {
  ...
  async execute(message, args, client, db) {
    ...
    if (owner) await vc.permissionOverwrites.edit(owner, { Connect: true });
    for (const mid of managers) {
      const m = await guild.members.fetch(mid).catch(() => null);
      if (m?.voice?.channelId === vc.id) await vc.permissionOverwrites.edit(m, { Connect: true });
    }

    sendLog({ guild, configDB: db, type: 'lock', actor: member, channel: vc }).catch(() => {});

    return styledSend(message, {
      title: 'Success',
      emoji: e.lock.trim(),
      msg:   'Channel has been locked',
    });
  },
};
